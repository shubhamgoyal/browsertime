...
module.exports = {
  ...
  harFromEvents(events, options) {
    function populateEntryFromResponse(entry, response) {
      ...
      if (timing) {
        let blocked = Number((firstNonNegative([timing['dnsStart'], timing['connectStart'], timing['sendStart']])).toFixed(0));

        let dns = -1;
        if (timing['dnsStart'] >= 0) {
          dns = Number((timing['dnsEnd'] - timing['dnsStart']).toFixed(0));
        }
        let connect = -1;
        if (timing['connectStart'] >= 0) {
          connect = Number((timing['connectEnd'] - timing['connectStart']).toFixed(0));
        }
        let send = Number((timing['sendEnd'] - timing['sendStart']).toFixed(0));
        let wait = Number((timing['receiveHeadersEnd'] - timing['sendEnd']).toFixed(0));
        let receive = 0;

        let ssl = -1;
        if (timing['sslStart'] >= 0) {
          ssl = Number((timing['sslEnd'] - timing['sslStart']).toFixed(0));
        }
    ...
    for (let event of events) {
      ...
      switch (message.method) {
        ...
        case 'Network.loadingFinished': {
          ...
          timings.receive = Number(((params.timestamp - entry._requestSentTime) * 1000 - entry._receiveHeadersEnd).toFixed(0));
        ...
        case 'Page.loadEventFired': {
          ...
          if (params.timestamp && page._timestamp) {
            page.pageTimings.onLoad = Number(((params.timestamp - page._timestamp) * 1000).toFixed(0));
          }
        }
          break;
        ...
        case 'Page.domContentEventFired': {
          ...
          if (params.timestamp && page._timestamp) {
            page.pageTimings.onContentLoad = Number(((params.timestamp - page._timestamp) * 1000).toFixed(0));
          }
        }
          break;
